...
def make_point_trace(df:pd.DataFrame,name:str)->go.Scattermapbox:
    ...
    #CASO ESPECIAL: se nao existirem valores numericos validos ou se todos os custos forem praticamente iguais (diferença < 1e-9) cria um array de tamanhos fixos 10 para todos os pontos
...
def main():
    ...
    fig.write_html(
        f"{folder}mapa_custos_interativo.html", include_plotlyjs = "cdn",full_html = True
        )
